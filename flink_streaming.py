...
def word_count(input_path, output_path): 
    ...
    if input_path is not None: 
        ...
        print("Executing word count with input dataset.")
        print("Use --input to specify file input.")
        ds = env.from_collection(data)

    def split(line): 
        yield from line.split()
    ds = ds.flat_map(split).map(lambda i: (i,1), output_type = Types.TUPLE([Types.STRING(), Types.INT()])).key_by(lambda i: i[0]).reduce(lambda i,j: (i[0], i[1] + j[1]))
    if output_path is not None: 
        ...
        print("Printing result. Use --output to specify output path")
        ds.print()
    env.execute()
